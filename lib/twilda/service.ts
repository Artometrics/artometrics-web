...
import type { Database } from "@/lib/twilda/database.types";
import type { CodexEntry, CoverKind, Novel } from "@/lib/twilda/novelcrafter/data";
import {
  ensureDefaultDraft,
  getActiveDraftId,
  listDrafts,
 ...
 * Starters are seeded with cover_kind in (gatsby, trinity, cardinal, apocrypha, artometrics, psychonomics) and is_template=true —
...
export async function createNovel(
  ...
  if (isTrinityNovel(title, coverKind)) {
    const { ensureTrinityDrafts } = await import("@/lib/twilda/starters");
