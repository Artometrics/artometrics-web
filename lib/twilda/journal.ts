...
export type JournalEntry = Database["public"]["Tables"]["twilda_journal_entries"]["Row"];

export async function listJournalEntries(supabase: Client, userId: string) {
  const { data, error } = await supabase
    .from("twilda_journal_entries")
...
export async function updateJournalEntry(
  ...
  const patch: Database["public"]["Tables"]["twilda_journal_entries"]["Update"] = {
...
export function isJournalSetupError(message: string): boolean {
  return /twilda_journal_entries|does not exist|schema cache|relation/i.test(message);
}
